# InsightLink - 智能文件追踪平台

## 项目简介

InsightLink 是一个智能文件追踪平台，专注于解决文件分享后的"信息黑洞"问题。通过为每个分享创建唯一的追踪链接，用户可以实时了解：
- 📊 谁查看了文件
- ⏰ 何时查看
- 👀 查看了多久
- 📍 访客的设备和位置信息
...
1.  **准备环境**：服务器需安装 Node.js 18+ 和 PostgreSQL。
2.  **上传代码**：将项目代码上传至服务器。
3.  **安装依赖**：
    ```bash
    npm install
...
6.  **启动服务**：
    ...
    npm start
    ```
    建议使用 `pm2` 进行进程守护：`pm2 start npm --name "insightlink" -- start`

### Docker 部署

```bash
docker build -t insightlink .
docker run -d -p 3000:3000 --env-file .env insightlink
```
